...
| `vrr_record.als` | Alloy model of the public/private boundary of a **verifiable research record** (D9; build plan M9). The other models are about a release; this one is about the RECORD of one. Checks that nothing nameable from the exported bundle is a stage the gateway refused, that published evidence has authorised release lineage, that `TRE_PRECOMMITTED` needs both chain order and chain authenticity (#108), that a stale certificate cannot authenticate, and that a private branch cannot enter the public topology. Its properties are stated over what a READER can recover rather than over which field the compiler wrote, because round 14's three findings were all fields correctly omitted from one place and recoverable from another; it machine-exhibits all three (#108, #109, #110) plus the stale certificate and the branch-visible topology, each with one clause of the compiler dropped. |
