...
def test_a_cohort_of_nobody_looks_like_a_cohort_of_one(service, tables):
    ...
    ages = donors.age_years.value_counts()
    singleton = int(ages[ages == 1].index[0])
    target = donors[donors.age_years == singleton].iloc[0]
    other_sex = "F" if target.sex == "M" else "M"

    hit = probe(service, SessionAuditor(budget=99),
                [{"column": "age_years", "op": "==", "value": singleton},
                 {"column": "sex", "op": "==", "value": target.sex}])
    miss = probe(service, SessionAuditor(budget=99),
                 [{"column": "age_years", "op": "==", "value": singleton},
                  {"column": "sex", "op": "==", "value": other_sex}])
...
def test_a_refusal_carries_no_number_from_the_data(service, tables):
    ...
    shapes = [
        [{"column": "age_years", "op": "==", "value": 99}],            # nobody
