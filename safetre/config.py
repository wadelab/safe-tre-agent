...
class PolicyConfig:
    ...
    response_quantum_ms: int = _dial(
        ...
        means="a response is held until the next multiple of this many "
              ...
              "indistinguishable by latency. What it has to cover is every "
              "cohort whose EXACT size is withheld — which is not only the "
              "sub-threshold ones (hardening #72). A released count is "
              "published ROUNDED, so two released cohorts inside one rounding "
              "bucket also have exact sizes the analyst does not hold, and "
              "latency separating them would refine a released count past the "
              "blur #26 to #28 exist to keep. The sizing argument used to say "
              "supra-threshold counts were 'published anyway', which is true "
              "only up to that rounding. Measured, the spread across every "
              "such cohort sits within a few milliseconds, so 50 puts them in "
              "one bucket with room; the number is unchanged, the reason for "
              "it is now the right one. Set to 0 to disable, which reopens "
              "the channel.",
