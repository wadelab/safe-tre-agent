...
% docs/elif.md (the cells-first model account), formal/ (the Alloy model).
...
finite request space of every procedure is exported as data and checked by a
bounded model in a solver. A red-team harness replays twenty-two scenarios with
...
might miss: the query skeleton, and all 718 points of the model skeleton, are
walked in full. A red-team harness exercises the prohibitions end to end
(\S\ref{sec:redteam}). Finally, the layer the earlier design only anticipated is
now real: each procedure exports its finite request space as data, and a bounded
model of the release path, generated from that export, is checked in a solver on
every point --- that suppressing any design cell denies the model, and that a
model is a function of finalized cells alone. Two synchronisation tests pin code
to the exported skeleton and the skeleton to the model, so a change to the code
that outran the proof would fail the build, and the solver runs in continuous
integration against a version-pinned binary.
